...
<asp:Content ID="Content5" runat="server" ContentPlaceHolderID="ContentPlaceHolder1">
    ...
    <head>
        ...
        <title>Register | RK University Placement Portal</title>
        ...
        <div class="container">
            <div class="register-container-box">
                ...
                <div class="right-col">
                    ...
                    <div class="mb-4 d-flex gap-4 border-bottom pb-3">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="accountType" id="typeStudent" value="student" checked onchange="toggleAccountType()">
                            <label class="form-check-label fw-bold text-dark" for="typeStudent">Student Registration</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="accountType" id="typeCompany" value="company" onchange="toggleAccountType()">
                            <label class="form-check-label fw-bold text-dark" for="typeCompany">Company Registration</label>
                        </div>
                    </div>
...
<asp:Content ID="Content7" runat="server" ContentPlaceHolderID="ContentPlaceHolder3">
    ...
    <script>
        ...
        // Handle Registration
        document.getElementById('btnSubmitRegister').addEventListener('click', function (e) {
            e.preventDefault();
            ...
            const result = PortalDB.register({
                name: fullName,
                enrollment: rollNo, // For company this acts as HR name
                email: email,
                mobile: mobile,
                username: username,
                password: password,
                role: role, // 'student' or 'company'
                status: role === 'student' ? 'active' : 'company'
            });

            if (result.success) {
                if (role === 'student') {
                    // Post welcome announcement notification for student
                    PortalDB.addAnnouncement(
                        `Welcome to RK University Placement Cell, ${fullName}!`,
                        `Your student account (${rollNo}) has been registered successfully. An email notification has been sent to ${email}. You can now log in to your Student Panel, complete your profile, upload certificates, and apply for campus drives.`
                    );
                    showAlert(`📧 Student Registration Successful! Email confirmation sent to <strong>${email}</strong>. Redirecting to login page...`, 'success');
                } else {
                    showAlert(`🏢 Company Registration Successful! Welcome <strong>${fullName}</strong>. Redirecting to login page...`, 'success');
                }
